...
function showError(msg) {
  resultsDiv.innerHTML = `
    ...
    <div class="result-title" style="color: var(--text); -webkit-text-fill-color: var(--text);">Oops!</div>
    <p class="result-desc">${msg}</p>
    <a href="../index.html"><button class="btn-primary">Play Now</button></a>
  `;
}

if (!hash || hash.length < 2 || hash.length % 2 !== 0) {
  showError('You need to play a game to see results.');
} else {
  try {
    ...
    for (let i = 0; i < hash.length; i += 2) {
      ...
      if (!celeb) throw new Error(`Unknown celebrity index: ${celebIdx}`);
    ...
    showError('Something went wrong reading your results. Please play again.');
    console.error(e);
  }
}
